...
/** Port of v1.0 system prompt selection (M0). */
object SystemPromptBuilder {
    fun forModel(modelName: String, mode: ChatMode): String {
        val lower = modelName.lowercase(Locale.ROOT)
        if (lower.contains("eburon pro") || lower.contains("eburon-pro")) {
            return when (mode) {
                ChatMode.CHAT -> EBURON_PRO_CHAT
                ChatMode.CODING -> EBURON_PRO_CODING
            }
        }
        if (lower.contains("eburon")) {
            return when (mode) {
                ChatMode.CHAT -> EBURON_CHAT
                ChatMode.CODING -> EBURON_CODING
            }
        }
    ...
    private fun genericChatPrompt(id: ModelIdentity): String = """
        You are ${id.name} — ${id.description} — running fully offline on the user's Android phone in the LocalChat app.
        ...
        - If asked what model or AI you are, answer truthfully: ${id.name}, running offline in LocalChat
    ...
    private fun genericCodingPrompt(id: ModelIdentity): String = """
        You are ${id.name} — ${id.description} — running fully offline on the user's Android phone in LocalChat.
        ...
        - If asked what model you are, answer: ${id.name}, running offline in LocalChat
    """.trimIndent()

    private const val EBURON_PRO_CHAT =
        "You are eburon-pro, an advanced AI assistant with a warm, conversational personality. " +
            "Your responses should feel natural, human-like, and engaging. You run fully offline on the user's " +
            "Android phone in the LocalChat app.\n\nAnswer in the same language the user writes in. " +
            "You are Eburon Pro running locally in LocalChat — not Claude, ChatGPT, or any cloud AI product."

    private const val EBURON_PRO_CODING =
        "You are Eburon Pro — a Belgian public-sector AI assistant based on Llama 3 — running fully offline " +
            "in LocalChat coding mode. Output complete runnable code in fenced blocks. " +
            "Answer in the same language the user writes in."

    private const val EBURON_CHAT =
        "You are EBURON-MEDIA-PRO, a high-performance assistant tailored for Creatives and Ads specialists.\n" +
    ...
    private const val EBURON_CODING =
        "You are Eburon, running offline in LocalChat coding mode.\n\nSTRICT output rules:\n" +
